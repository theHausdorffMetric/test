...
STATUS_MAPPING = {'On Order': 'Launched'}
...
def field_mapping():
    return {
        ...
        'ETA': ignore_key('eta'),
        'IMO / MMSI': ignore_key('retrieve from url, this field would not always exist'),
        'MMSI': ('mmsi', None),
        'Callsign': ('call_sign', _handle_empty_value),
        ...
        'IMO number': ('imo', None),
        'Vessel Name': ('name', lambda x: x.title()),
        'Ship type': ('type', None),
        ...
        'status': ('status', lambda x: STATUS_MAPPING.get(x, x)),
    }
